...
from envs_snake import create_env
...
args.num_actions = 5 # get the action space of this game
...
prepro = lambda img: cv2.resize(img, (80, 80), interpolation=cv2.INTER_AREA).astype(np.float32)/255.  # .reshape(3,80,80)  MsPacman-v4
...
def train(shared_model, shared_optimizer, rank, args, info):
    ...
    model = NNPolicy(channels=3, memsize=args.hidden, num_actions=args.num_actions).to(device=args.device) # a local/unshared model
    ...
    while info['frames'][0] <= 8e8 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        # for step in range(0, np.random.randint(10, 40)):
            episode_length += 1
            value, logit, hx = model((state.view(1, 3, 80, 80), hx.to(device=args.device)))
        ...
        next_value = torch.zeros(1,1).to(device=args.device) if done else model((state.view(1, 3, 80, 80), hx))[0]
...
if __name__ == "__main__":
    ...
    shared_model = NNPolicy(channels=3, memsize=args.hidden, num_actions=args.num_actions).to(args.device).share_memory()
...
# python snake.py --processes 4 --rnn_steps 30 --hidden 256