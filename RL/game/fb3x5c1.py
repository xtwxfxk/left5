...
args.save_dir = 'run/{}-3x5c1/'.format(args.env.lower()) # keep the directory structure simple
if args.render:
    args.processes = 1
...
class NNPolicy(nn.Module): # an actor-critic neural network
    def __init__(self, channels, memsize, num_actions):
        ...
        self.gru = nn.GRUCell(2 * 128, memsize)
    ...
    def forward(self, inputs, train=True, hard=False):
        ...
        x = torch.cat([x, self.seq1(inputs)])
        hx = self.gru(x.view(-1, 2 * 128), (hx))
