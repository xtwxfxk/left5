...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    Args:
        ...
        truths: (tensor) Ground truth boxes, Shape: [num_obj, 4]. # [num_obj, num_priors].
    ...
    # print('#####')
    # print(np.unique(best_truth_idx.cpu().numpy()))
    # print(np.unique(best_prior_idx.cpu().numpy()))
    # print('#####')
    best_truth_overlap.index_fill_(0, best_prior_idx, 2)  # ensure best prior
    # TODO refactor: index  best_prior_idx with long tensor
    # ensure every gt matches with its prior of max overlap
    for j in range(best_prior_idx.size(0)): # j: label
        # print(best_prior_idx[j])
...
def encode(matched, priors, variances):
    ...
    Args:
        ...
        priors: (tensor) Prior boxes in center-offset form
            Shape: [num_priors, 4].
        variances: (list[float]) Variances of priorboxes # [.1, .2]
    Return:
