...
from utils import load_symbols, EXCHANGE_REGEX

logger = logging.getLogger('verbose')

class CFtdcMdSpi(mdapi.CThostFtdcMdSpi):
    tapi=''
    def __init__(self, tapi, broker_id, user_id, password, save_dir, data_queue):
    ...
    def OnRtnDepthMarketData(self, *args):
        try:
            ...
            day_dir = os.path.join(self.save_dir, '%s_json' % today)
            ...
            if exchange:
                file_path = os.path.join(day_dir, '%s.%s' % (exchange, instrumentID))
            else:
                file_path = os.path.join(day_dir, instrumentID)
...
def ctp_process(front_addr, broker_id, user_id, password, save_dir, data_queue):

    mduserapi = mdapi.CThostFtdcMdApi_CreateFtdcMdApi()
    logger.info("版本号为: %s" % mduserapi.GetApiVersion())
    mduserspi = CFtdcMdSpi(mduserapi, broker_id, user_id, password, save_dir, data_queue)
