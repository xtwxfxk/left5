# -*- coding: utf-8 -*-
import os, datetime, json, itertools, traceback
import threading
import queue
...
from utils import target_call_put, load_symbols_b
...
class CFtdcMdSpi(mdapi.CThostFtdcMdSpi):
    tapi=''
    def __init__(self, tapi, broker_id, user_id, password):
        mdapi.CThostFtdcMdSpi.__init__(self)
        self.tapi = tapi
        self.BROKERID = broker_id
        self.USERID = user_id
        self.PASSWORD = password
        # self.symbols = [b'rb2309', b'rb2309C3850', b'rb2309P3850'] # 

        symbols = []
        tcp = target_call_put()
        for k, v in tcp.items():
            symbols.append(k.encode('utf-8'))
            symbols.extend([s.encode('utf-8') for s in v['call']])
            symbols.extend([s.encode('utf-8') for s in v['put']])


        # self.symbols = [c.encode('utf-8') for c in list(itertools.chain.from_iterable(columns))]
        # self.symbols = load_symbols_b() # [line.strip().encode('utf-8') for line in open('symbols.txt', 'r') if line.strip()] # 
...
def ctp_process(front_addr, broker_id, user_id, password):

    mduserapi = mdapi.CThostFtdcMdApi_CreateFtdcMdApi()
    logger.info("版本号为: %s" % mduserapi.GetApiVersion())
    mduserspi = CFtdcMdSpi(mduserapi, broker_id, user_id, password)
