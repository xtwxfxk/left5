...
class Strategy:
    ...
    def begin(self, obs):
        ...
        action, self.state = self.model.predict(obs, state=self.state, deterministic=True)
        action_type = action[0]
        amount = action[1]
        print(action)
        if action_type < 1 and self.backtest.long_quantity == 0:
            self.backtest.buy(
        ...
        elif action_type < 2 and self.backtest.long_quantity !=0:
