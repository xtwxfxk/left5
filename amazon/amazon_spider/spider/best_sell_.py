    ...
    def wrapped(self, **kwargs):
        ...
        if category_eles is not None and len(category_eles) > 0:
            for category_ele in category_eles:
                ...
                if self.session.query(Url).filter_by(key=key).count() < 1:
...
def product_url(method):
    def wrapped(self, **kwargs):
        ...
        for ele in product_eles:
            ...
            if self.session.query(Url).filter_by(key=asin).count() < 1:
...
def url_over(method):
    def wrapped(self, **kwargs):
        try:
            ...
            self.session.query(Url).filter_by(id=url_obj.id).update({Url.tries: url_obj.tries+1})
        else:
            self.session.query(Url).filter_by(id=url_obj.id).update({Url.has_crawled: True})
...
class BestSell(AmazonBase, threading.Thread):
    ...
    def run(self):
        while 1:
            try:
                _url_obj = self.queue.get(timeout=30)
