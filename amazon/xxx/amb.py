...
import os, time, csv, traceback, queue, configparser
...
thread_num = int(config['setting']['thread_num'])
...
def start():
    ...
    try:
        ...
        for i in range(thread_num):
            executor.submit(SpiderAmazon(q, os.path.join(user_data_root, 'amazon%s' % i)).do)
