...
class StockTradingEnvCashpenalty(gym.Env):
    ...
    def return_terminal(self, reason="Last Date", reward=0):
        ...
        tf_logger.log("environment/GainLoss_pct", (gl_pct - 1) * 100)
        tf_logger.log(
            "environment/total_assets",
            int(self.account_information["total_assets"][-1]),
        )
        reward_pct = self.account_information["total_assets"][-1] / self.initial_amount
        tf_logger.log("environment/total_reward_pct", (reward_pct - 1) * 100)
        tf_logger.log("environment/total_trades", self.sum_trades)
        tf_logger.log(
        ...
        tf_logger.log("environment/completed_steps", self.current_step)
        tf_logger.log(
    ...
    def log_step(self, reason, terminal_reward=None):
        ...
        logger.log(self.template.format(*rec))
    ...
    def step(self, actions):
        ...
        # actions = np.nan_to_num(actions, 0)
            ...
            if (spend + costs) > coh:
                if self.patient:
                    ...
                    return self.return_terminal(
                        reason="CASH SHORTAGE", reward=reward # self.get_reward()
                    )
